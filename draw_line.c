...
/*   Updated: 2018/01/03 22:11:59 by omiroshn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	line_process_point(t_mapinfo *map, t_bresenhem *a, t_vec *vec1, t_vec *vec2)
{
	double percent;
	double val  = sqrt((a->distx * a->distx) + (a->disty * a->disty));
	percent = (a->distx > a->disty ?
			ft_ilerp(val, a->x0, a->x1)
			: ft_ilerp(val, a->y0, a->y1));
	mlx_pixel_put(map->mlx, map->win, a->x0 + WIDTH / 2,
			a->y0 + HEIGHT / 2, clerp(vec1->color, vec2->color, 0.1));
    a->err2 = 2 * a->err;
    if (a->err2 >= a->disty)
    {
    	a->err += a->disty;
...
void	draw_line(t_mapinfo *map, t_vec vec1, t_vec vec2)
{
	...
	a.x1 = (int)vec2.x;
	a.y1 = (int)vec2.y;
	a.distx =  abs (a.x1 - (int)vec1.x);
	a.disty =  - abs (a.y1 - a.y0);
	a.sx = (int)vec1.x < a.x1 ? 1 : -1;
	a.sy = a.y0 < a.y1 ? 1 : -1;
	a.err = a.distx + a.disty;
	while (a.x0 != a.x1 || a.y0 != a.y1)
		line_process_point(map, &a, &vec1, &vec2);
}
